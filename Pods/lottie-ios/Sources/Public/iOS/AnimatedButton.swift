...
open class AnimatedButton: AnimatedControl {
  ...
  open override func beginTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
    ...
    if let playrange = rangesForEvents[touchEvent.rawValue] {
      animationView2.play(fromProgress: playrange.from, toProgress: playrange.to, loopMode: LottieLoopMode.playOnce)
    }
    return true
  }
  ...
  public func setPlayRange(fromMarker fromName: String, toMarker toName: String, event: UIControl.Event) {
    if
      let start = animationView2.progressTime(forMarker: fromName),
      let end = animationView2.progressTime(forMarker: toName)
    {
