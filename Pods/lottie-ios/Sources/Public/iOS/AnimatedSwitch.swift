...
open class AnimatedSwitch: AnimatedControl {
  ...
  func updateOnState(isOn: Bool, animated: Bool, shouldFireHaptics: Bool) {
    ...
    if cancelBehavior == .reverse {
      let realtimeProgress = animationView2.realtimeAnimationProgress
    ...
    guard animated == true else {
      animationView2.currentProgress = finalProgress
      return
    }
    ...
    animationView2.play(
      ...
      completion: { [weak self] finished in
        ...
        if finished, !(self.animationView2.animationLayer is CoreAnimationLayer) {
          self.animationView2.currentProgress = finalProgress
        }
      })
  }
