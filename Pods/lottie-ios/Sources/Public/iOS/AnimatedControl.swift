...
open class AnimatedControl: UIControl {
  ...
  {
    animationView2 = LottieAnimationView(
      animation: animation,
  ...
  open override var intrinsicContentSize: CGSize {
    animationView2.intrinsicContentSize
  }
  ...
  public var animationSpeed: CGFloat {
    set { animationView2.animationSpeed = newValue }
    get { animationView2.animationSpeed }
  }
  ...
  public func setValueProvider(_ valueProvider: AnyValueProvider, keypath: AnimationKeypath) {
    animationView2.setValueProvider(valueProvider, keypath: keypath)
  }
  ...
  func updateForState() {
    guard let animationLayer = animationView2.animationLayer else { return }
    if
  ...
  private func commonInit() {
    animationView2.clipsToBounds = false
    clipsToBounds = true
    animationView2.translatesAutoresizingMaskIntoConstraints = false
    animationView2.backgroundBehavior = .forceFinish
    addSubview(animationView2)
    animationView2.contentMode = .scaleAspectFit
    animationView2.isUserInteractionEnabled = false
    animationView2.leadingAnchor.constraint(equalTo: leadingAnchor).isActive = true
    animationView2.trailingAnchor.constraint(equalTo: trailingAnchor).isActive = true
    animationView2.topAnchor.constraint(equalTo: topAnchor).isActive = true
    animationView2.bottomAnchor.constraint(equalTo: bottomAnchor).isActive = true
  }
}
#endif
